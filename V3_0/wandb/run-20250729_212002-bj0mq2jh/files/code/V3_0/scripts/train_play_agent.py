...
def main() -> None:
    ...
    policy_kwargs = dict(
        net_arch=[256, 256],
        activation_fn=nn.ReLU,  # Doğru: class olarak verilmeli!
    )

    model = DQN(
        "MlpPolicy",
        vec_env,
        learning_rate=linear_schedule(1e-3, 1e-4),  # Daha yüksek başlangıç LR
        exploration_fraction=0.3,  # Daha uzun exploration (was 0.08)
        exploration_final_eps=0.05,  # Biraz daha yüksek final eps
        exploration_initial_eps=1.0,
        buffer_size=100_000,
        batch_size=1024,  # Daha küçük batch size
        target_update_interval=10_000 // args.n_envs,
        gamma=0.99,  # Düzeltildi: was 1.0, now 0.99
        train_freq=4,  # Daha sık training
        gradient_steps=1,  # Her train_freq'te 1 gradient step
